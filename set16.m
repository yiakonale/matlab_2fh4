...
Rin=1.5; %inner radius of toroid
Rout=2.5; %outer radius of toroid
Ravg=(Rin+Rout)/2;
r=(Rout-Rin)/2; %internal radius of toroid

Phimin=0; %coordinate of the lowest point on the solenoid
Phimax=2*pi; %coordinate of the highest point on the solenoid
...
NumberOfSegments=100; %we divide the toroid into this number of segments
t_values=linspace(t_min,t_max,(NumberOfSegments+1))'; %these are the values of the parameter t
phi_values=Phimin+((Phimax-Phimin)/(t_max-t_min))*(t_values-t_min);
...
PlotXmin=-4.0; %lowest x value on the plot plane
PlotXmax=4.0; %maximum x value on the plot plane
PlotYmin=-4.0; %lowest y value on the plot plane
PlotYmax=4.0; %maximum y value on the plot plane
